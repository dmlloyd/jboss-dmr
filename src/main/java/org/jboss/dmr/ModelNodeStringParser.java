 ...
 */
class ModelNodeStringParser {
    final TrackingReader reader;

    ModelNodeStringParser(final TrackingReader reader) {
        this.reader = reader;
    }
    ...
    ModelNode parseModelNode() throws IOException {
        ...
        switch (cp) {
                ...
                switch (cp) {
                    case 'I': {
                        ...
                        if (cp == 'D') {
                            ...
                            requireNonWord();
                            return new ModelNode(ModelType.BIG_DECIMAL);
                        } else if (cp == 'I') {
                            requireString("NTEGER");
                            requireNonWord();
                            return new ModelNode(ModelType.BIG_INTEGER);
                        } else {
                    ...
                    case 'Y': {
                        requireString("TES");
                        requireNonWord();
                        return new ModelNode(ModelType.BYTES);
                    }
                    case 'O': {
                        requireString("OLEAN");
                        requireNonWord();
                        return new ModelNode(ModelType.BOOLEAN);
                    }
                    default: {
                        throw unexpectedCharacter(cp);
                    }
            ...
            case 'D': {
                requireString("OUBLE");
                requireNonWord();
                return new ModelNode(ModelType.DOUBLE);
            }
            case 'E': {
                requireString("XPRESSION");
                requireNonWord();
                return new ModelNode(ModelType.EXPRESSION);
            }
            case 'I': {
                cp = reader.requireCodePoint();
                if (cp == 'n') {
                    requireString("finity");
                    requireNonWord();
                    return new ModelNode(Double.POSITIVE_INFINITY);
                } else if (cp == 'N') {
                    requireString("T");
                    requireNonWord();
                    return new ModelNode(ModelType.INT);
                } else {
                    throw unexpectedCharacter(cp);
                }
            }
            case 'L': {
                ...
                if (cp == 'I') {
                    requireString("ST");
                    requireNonWord();
                    return new ModelNode(ModelType.LIST);
                } else if (cp == 'O') {
                    requireString("NG");
                    requireNonWord();
                    return new ModelNode(ModelType.LONG);
                } else {
                    throw unexpectedCharacter(cp);
                }
            }
            ...
            case 'P': {
                requireString("ROPERTY");
                requireNonWord();
                return new ModelNode(ModelType.PROPERTY);
            }
            case 'S': {
                requireString("TRING");
                requireNonWord();
                return new ModelNode(ModelType.STRING);
            }
            case 'T': {
                requireString("YPE");
                requireNonWord();
                return new ModelNode(ModelType.TYPE);
            }
            case 'U': {
                requireString("NDEFINED");
                requireNonWord();
